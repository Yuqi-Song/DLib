...
from tool.attackConfig import attackLineConfig
...
class FileIO(object):
    ...
    @staticmethod
    def loadRate(conf, file, bTest=False):
        trainingData = defaultdict(dict)
        testData = defaultdict(dict)
        ratingConfig = attackLineConfig(conf['ratings.setup'])
        if not bTest:
    ...
    def loadRelationship(conf, filePath):
        socialConfig = attackLineConfig(conf['social.setup'])
        relation = []
